      ...
      <div className="space-y-4">
        ...
        {sections.length === 0 && (
          <div className="text-center py-6">
            <p className="text-sm text-gray-300 mb-3">No weeks found yet — the sheet connected fine, but nothing in it matched the expected layout.</p>
            {debug && (
              <details className="text-left bg-gray-50 rounded-lg p-3 max-w-2xl mx-auto">
                <summary className="text-xs font-semibold text-gray-500 cursor-pointer">Show what's actually being read from the sheet (for debugging)</summary>
                <pre className="text-[10px] text-gray-500 mt-2 overflow-x-auto whitespace-pre-wrap">{JSON.stringify(debug, null, 2)}</pre>
              </details>
            )}
          </div>
        )}
                      ...
                      {TEAMS.map(team => {
                        ...
                        return (
                          <div key={team} onClick={() => isClickable && setSelectedProdTeam(isSelected ? null : team)}
                            className={`rounded-xl overflow-hidden border transition-all ${isClickable ? "cursor-pointer hover:shadow-sm" : "opacity-60"} ${isSelected ? "border-gray-300 shadow-sm ring-2 ring-gray-200" : "border-gray-100"}`} style={{ background: "#fff" }}>
...
const KPI_PACKAGE_FOLDERS = [
  ...
  { name: "Content Curation Team", url: "https://drive.google.com/drive/folders/1EUFh1vrkQaaPxeCmO3et51PJInAI16We", live: true, emoji: "\ud83d\udcdd", gradient: "from-emerald-500 to-teal-600" },
