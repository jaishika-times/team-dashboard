...
export function parseWeeklyKPI(buffer, filename) {
  const wb = XLSX.read(buffer, { type: "array", cellDates: true });
  // Find the KPI sheet
  const sn = wb.SheetNames.find(s => s.toLowerCase().includes("kpi") || s.toLowerCase().includes("weekly")) || wb.SheetNames[0];
  ...
  for (let i = 0; i < Math.min(5, rows.length); i++) {
    const row = rows[i];
    if (row?.some(c => String(c || "").toLowerCase().includes("employee") || String(c || "").toLowerCase() === "team")) { hIdx = i; break; }
  }
  if (hIdx === -1) throw new Error("Could not find header row with Employee/Team columns");
  ...
  const empCol = h.findIndex(x => x.includes("employee") || x === "name");
  const teamCol = h.findIndex(x => x === "team");
  const kpiTypeCol = h.findIndex(x => x.includes("kpi type") || x.includes("type"));
  const targetCol = h.findIndex(x => x.includes("target"));
  const notesCol = h.findIndex(x => x.includes("notes"));
  const actualCol = h.findIndex(x => x.includes("actual"));
  const kpiPctCol = h.findIndex(x => x.includes("kpi %") || x.includes("kpi%") || x.includes("percentage"));
  const statusCol = h.findIndex(x => x.includes("status"));

  if (empCol === -1) throw new Error("Could not find Employee column");
  ...
  for (let i = hIdx + 1; i < rows.length; i++) {
    ...
    const month = monthCol >= 0 && row[monthCol] ? String(row[monthCol]).trim() : lastMonth;
    const week = weekCol >= 0 && row[weekCol] ? String(row[weekCol]).trim() : lastWeek;
    const emp = empCol >= 0 ? String(row[empCol] || "").trim() : "";
    const team = teamCol >= 0 ? String(row[teamCol] || "").trim() : "";
    const kpiType = kpiTypeCol >= 0 ? String(row[kpiTypeCol] || "").trim() : "";
    const target = targetCol >= 0 ? row[targetCol] : "";
    const notes = notesCol >= 0 ? String(row[notesCol] || "").trim() : "";
    const actual = actualCol >= 0 ? row[actualCol] : "";
    const kpiPct = kpiPctCol >= 0 ? parseFloat(row[kpiPctCol]) : null;
    ...
    entries.push({
      month: lastMonth, week: lastWeek, employee: emp, team,
      kpiType, target: String(target || ""), notes,
      actual: String(actual || ""), kpiPct: isNaN(kpiPct) ? null : kpiPct, status,
    });
  }
  ...
  const grouped = {};
  entries.forEach(e => {
    const key = e.month + " W" + e.week;
    if (!grouped[key]) grouped[key] = { month: e.month, week: e.week, teams: {} };
    if (!grouped[key].teams[e.team]) grouped[key].teams[e.team] = [];
    grouped[key].teams[e.team].push(e);
  });

  const periods = Object.keys(grouped).reverse();
  return { entries, grouped, periods, count: entries.length };
}
