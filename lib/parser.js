...
export function parseInOutReport(buffer, filename) {
  const wb = XLSX.read(buffer, { type: "array", cellDates: true });
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    // Last check-out (scan backwards for last non-empty)
    ...
    // Total hours = last out - first in, minus 1hr lunch if > 5hrs
    let th = 0;
    if (cim !== null && lastOutTime !== null && lastOutTime > cim) {
      th = (lastOutTime - cim) / 60;
      if (th > 5) th -= 1;
    }
    ...
    // Short hours (only if they worked)
    ...
    wR[wn].push({ date: pd, name, ci: firstIn || "-", co: lastOut || "-", hrs: hd, rm: "" });
  }
...
export function parseAttendanceAuto(buffer, filename) {
  const wb = XLSX.read(buffer, { type: "array", cellDates: true });
  const sn = wb.SheetNames[0];
  const rows = XLSX.utils.sheet_to_json(wb.Sheets[sn], { header: 1, raw: false });
  const header = rows[0]?.map(h => String(h || "").toLowerCase().trim()) || [];
  ...
  if (hasCheckIn1) {
    // Raw In/Out Report from HR system
    return parseInOutReport(buffer, filename);
  } else {
    // Original ATT_Month format
