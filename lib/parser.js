...
function parseAttendanceSummary(wb, filename) {
  ...
  const summarySheet = wb.SheetNames.find(s => 
    s.toLowerCase().includes("summary") || s.toLowerCase().includes("attendance summary")
  );
  if (!summarySheet) return null;

  const rows = XLSX.utils.sheet_to_json(wb.Sheets[summarySheet], { header: 1, raw: true });
  ...
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    if (!row || row.every(c => !c && c !== 0)) { headerFound = false; continue; }
    
    // Join all cells to check for section headers
    const rowText = row.map(c => String(c || "")).join(" ").toLowerCase();
    
    if (rowText.includes("late clock-in") || rowText.includes("late clock in")) {
      section = "late"; headerFound = false; continue;
    }
    if (rowText.includes("short hour")) {
      section = "short"; headerFound = false; continue;
    }
    if (rowText.includes("sl/el") || rowText.includes("sl / el") || rowText.includes("frequent sl")) {
      section = "sle"; headerFound = false; continue;
    }

    // Skip header rows (Employee Name, Count, etc.)
    if (rowText.includes("employee name") || rowText.includes("employee_name")) {
      headerFound = true; continue;
    }

    if (!section || !headerFound) continue;

    // Find the first non-empty cell as the name
    let name = null;
    let values = [];
    ...
    // Skip if name looks like a number only
    if (!isNaN(name)) continue;

    if (section === "late" && values.length >= 1) {
      late.push({ name, count: Math.round(values[0]) });
    } else if (section === "short" && values.length >= 1) {
      short.push({ name, count: Math.round(values[0]) });
    } else if (section === "sle") {
      sle.push({ name, sl: Math.round(values[0] || 0), el: Math.round(values[1] || 0) });
    }
  }
  ...
  wb.SheetNames.forEach(sn => {
    ...
    for (let i = hIdx + 1; i < json.length; i++) {
      ...
      let pd = String(lastDate);
      const dm = pd.match(/(\d{1,2})[\/-](\d{1,2})/);
      if (dm) {
